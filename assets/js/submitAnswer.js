...
$(".body-1").on("click", ".btn-key", function () {
    database.ref("players/" + player.key).update({myAnswer: ($(this).data('integer'))})
})
        ...
        .then(function (snap) {
            ...
            snap.forEach(function (childSnapshot) {
                ...
                if (playerNum === 1) {
                    ...
                    playerOneInfo.wins = wins;
                    playerOneInfo.losses = losses;
                } else {
                    playerTwoInfo.answer = answer;
                    playerTwoInfo.key = playerKey;
                    playerTwoInfo.wins = wins;
                    playerTwoInfo.losses = losses;
                }
            ...
            if (numberOfPlayers === 2 && playerOneInfo.answer !== 0 && playerTwoInfo.answer !== 0) {
                if (playerOneInfo.answer === 1 && playerTwoInfo.answer === 2) { //rock vs paper
                    //player 2 wins
                    database.ref("players/" + playerOneInfo.key).update({losses: playerOneInfo.losses + 1})
                    database.ref("players/" + playerTwoInfo.key).update({wins: playerTwoInfo.wins + 1})
                } else if (playerOneInfo.answer === 1 && playerTwoInfo.answer === 3) { //rock vs scissors
                    //player 1 wins
                    database.ref("players/" + playerOneInfo.key).update({wins: playerOneInfo.wins + 1})
                    database.ref("players/" + playerTwoInfo.key).update({losses: playerTwoInfo.losses + 1})
